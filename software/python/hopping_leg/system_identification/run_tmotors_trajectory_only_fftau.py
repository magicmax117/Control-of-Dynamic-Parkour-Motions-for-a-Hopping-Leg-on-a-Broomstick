...
motor_elbow_id = 0x09
Kp_shoulder = 0.0                                                            # motor gains
Kd_shoulder = 0.0
Kp_elbow = 0.0
Kd_elbow = 0.0
provided_tau = "only_fftau"   # fftau_from_sys_id, fftau_from_cad, zero_fftau

# define input and output directories
experiment = "20220324_param_id_excitation_traj_202406"
input_directory = "../../results/20220324_param_id_excitation_traj_202406/test_data/trajectory-pos-50-3x/gains_50.0kp_0.5kd_fftau_from_cad/"
input_file = "trajectory-pos-50-3x_measured"        # cycloidal_trajectory_500Hz
suffix = ".csv"
data = pd.read_csv(os.path.join(input_directory, input_file + suffix))
output_directory = "../../results/" + experiment + "/test_data/" + \
                    f'{input_file}' + "/gains_" + f'{Kp_shoulder}' + "kp_" + \
                    f'{Kd_shoulder}' + "kd_" + f'{provided_tau}'
...
try:
    ...
    for i in range(numSteps):
        ...
        shoulder_pos, shoulder_vel, shoulder_tau = \
            motor_shoulder_controller.send_rad_command(0,    # shoulder_pos_traj[i]
                                                       0,    # shoulder_vel_traj[i]
                                                       0,             # Kp_shoulder
                                                       0,             # Kd_shoulder
                                                       shoulder_tau_traj[i])   # shoulder_tau_traj[i] send pos, vel and tau_ff command and use the in-built low level controller
        elbow_pos, elbow_vel, elbow_tau = \
            motor_elbow_controller.send_rad_command(0,
                                                    0,
                                                    0,
                                                    0,
                                                    elbow_tau_traj[i])         # elbow_tau_traj[i]
