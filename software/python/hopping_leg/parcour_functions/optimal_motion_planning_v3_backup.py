...
def motion_planning():
    ...
    h1 = 0.2
    obstacle_1_polynom, hurdle_1_polynom, platform_1_polynom = hurdle_builder(position=0.6, height=h1,
    ...
    for i in range(N):
        ...
        for j in range(fide - 1):
            x_c = x0 + vx * dh * (j + 1) - r_f * np.cos(theta_f)
            z_foot = z0 + vz * dh * (j + 1) - 0.5 * g * dh * (j + 1) * dh * (j + 1) - dz_hip_foot
            f1 = hurdle_1_polynom(x_c)
            prog.AddConstraint(hurdle_1[fide * i + j] >= f1)
            prog.AddConstraint(hurdle_1[fide * i + j] * f1 >= 0)
            p_foot = h1 * hurdle_1[fide * i + j]
