...
prog.AddBoundingBoxConstraint(0.01, 1, t)
...
for i in range(N):
    ...
    if i == N - 1:
        ...
        prog.AddConstraint(z <= p + 1e-3)
        x0 = x0 + x[i] + r * np.cos(theta[i+1])
        z0 = z + r * np.sin(theta[i+1])
...
for i in range(N):
    ...
    X_c = X_c + X[-1] + r * np.cos(solver.GetSolution(theta[i]))
    z_start = Z[-1] + r * np.sin(solver.GetSolution(theta[i]))
ax.plot(0, 0, 'or')
ax.plot(xg, zg, 'xr')

parkour_function_x = np.arange(x_start, xg + 0.01, 0.01)
